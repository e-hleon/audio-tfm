...
@Composable private fun HistoryScreen(url: String, backend: BackendRepository) {
    val vm = remember { HistoryViewModel(backend) }; val state by vm.state.collectAsStateWithLifecycle(); Column(Modifier.verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        Button({ vm.load(url) }) { Text("Cargar histórico") }; when (val current = state) { HistoryState.Idle -> Text("Pulsa cargar histórico"); HistoryState.Loading -> LinearProgressIndicator(); is HistoryState.Error -> { Text(current.message); OutlinedButton({ vm.load(url) }) { Text("Reintentar") } }; is HistoryState.Ready -> if (current.items.isEmpty()) Text("No hay interacciones") else current.items.forEach { item -> Text(item.recordedAt); Text(item.analysis.summary); Text(item.analysis.topics.joinToString(", ")); Spacer(Modifier.height(8.dp)) } }
    }
}
